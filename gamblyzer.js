  ...
  try {
    for (const sport of selectedSports) {
      try {
        ...
        // This variable can only be declared once!
...
async function generateNarrative(bet, claudeKey) {
  const stop = startSpinner('Claude is researching recent news…');
  try {
    const pt = bet.point !== undefined ? ` (${bet.point >= 0 ? '+' : ''}${bet.point})` : '';
...
async function pickFlow(keys) {
  ...
  while (keepGoing) {
    ...
    let narrative = '';
    try {
      narrative = await generateNarrative(bet, keys.claudeKey);
