...
/** VegasInsider line-movements path uses away-vs-home nickname slugs (see Tier 6). */
function vegasInsiderSportPath(sportLabel) {
  const s = String(sportLabel || "").toUpperCase().trim();
  if (s === "MLB") return "mlb";
  if (s === "NBA") return "nba";
  if (s === "NHL") return "nhl";
  return null;
...
function teamNameToVegasInsiderSlug(teamName) {
  ...
  if (words.length >= 2) {
    ...
    if (twoWordNicknames.has(last2)) return words.slice(-2).join("-");
  }
  return words[words.length - 1] || "";
}

function buildVegasInsiderLineMovementsUrl(pick) {
  const sport = vegasInsiderSportPath(pick?.sportLabel);
  if (!sport) return null;
  const awaySlug = teamNameToVegasInsiderSlug(pick?.fixture?.participant2Name);
  const homeSlug = teamNameToVegasInsiderSlug(pick?.fixture?.participant1Name);
  if (!awaySlug || !homeSlug) return null;
  return `https://www.vegasinsider.com/${sport}/matchups/${awaySlug}-vs-${homeSlug}/line-movements/`;
}
...
function buildGroundingBlock(pick) {
  ...
  const viLineMovements = buildVegasInsiderLineMovementsUrl(pick);
  const viLine =
    viLineMovements != null
      ? `- Line movements (VegasInsider — cite this URL for open-to-current / RLM): ${viLineMovements}`
      : "";
...
TIER 6 — Betting Market Structure (check on every bet)
  ...
  6A — PUBLIC BETTING SPLITS (mandatory)
  SOURCE (public, no paywall):
    DraftKings Network betting splits
    https://dknetwork.draftkings.com/draftkings-sportsbook-betting-splits/
    — per-game tables for Moneyline, Spread, and Total with **% Bets** (ticket count share)
    and **% Handle** (share of money wagered). Filter by sport/date; open the row for
    THIS matchup and the market that matches the posted bet (ML / spread / total).

  CITATION (6A): % Bets, % Handle belong **only** in FACT lines whose URL is the DK Network
    splits page. Quote exact figures from the table row for the relevant outcome. If the page
    or this game/market is missing, GAP Tier 6A — do not invent percentages.

  WHAT TO LOOK FOR (6A):
    % Bets vs. % Handle divergence (sharp vs public)
    Public lean thresholds: >65% Bets = lean; >80% = fade candidate
    Match market to ticket (ML / spread / total row for the posted wager)

  6B — LINE MOVEMENTS / OPEN-TO-CURRENT (mandatory for MLB, NBA, NHL)
  SOURCE — use **only** this URL pattern (public VegasInsider line-history table):
    https://www.vegasinsider.com/{sport}/matchups/{away-team-slug}-vs-{home-team-slug}/line-movements/

  Build the URL from grounding:
    - {sport}: lowercase league path — **mlb** | **nba** | **nhl** (from Sport in grounding)
    - Slug order: **away** team slug first, **home** second — same as "Away @ Home" in grounding
    - Team slugs: lowercase nickname, hyphenate two-word nicknames (e.g. marlins, rays,
      trail-blazers, red-sox, maple-leafs)
    - Example (MLB): https://www.vegasinsider.com/mlb/matchups/marlins-vs-rays/line-movements/

  If GROUNDING includes a pre-built VegasInsider line-movements URL, open **that exact URL**
  first (do not guess a different slug spelling).

  CITATION (6B): Moneyline, total, and runline/puckline rows with timestamps/history belong
    **only** in FACT lines citing the VegasInsider line-movements page for this matchup.
    Quote the movement you see (e.g. opener vs current on the relevant market). If the page
    fails to load or the matchup slug 404s, GAP Tier 6B — do not invent line history.

  WHAT TO LOOK FOR (6B):
    Reverse line movement vs public splits (6A)
    Total number moves (e.g. 7 ↔ 7.5) and price steam on the side tied to the ticket
    Runline/spread price moves when the posted bet is spread/RL
...
- UNDERREPORTED = sourced from Tier 2–5 primary source; absent from aggregators
  (Tier 6 % Bets / % Handle splits: UNDERREPORTED when cited from DK Network betting
  splits unless pick sites already lead with that exact split today — then SATURATED)
...
OVER-PRICED: <narrative name>
  ...
  SIGNAL: SATURATED
  MARKET: <Tier 6 % Bets / % Handle or RLM note if available; omit if no Tier 6 data>
...
UNDERPRICED: <narrative name>
  ...
  Tier 6 findings (% Bets vs % Handle divergence, RLM) should always appear here if present.
