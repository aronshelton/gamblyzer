...
function biasNarrativePrompt(groundingFeedFacts, proResearch, contraResearch) {
  const pro = String(proResearch || "").trim() || "(none)";
  const contra = String(contraResearch || "").trim() || "(none)";
  return `Sports betting analyst. Identify narratives bettors are likely to encounter and over-price for THIS exact line. Use only facts already present in PRO RESEARCH and CONTRA RESEARCH — do not introduce new stats, injuries, or results.
...
OVER-PRICED: <narrative name>
  ...
  SIGNAL: SATURATED
  MARKET: <bet%/money% split or RLM if stated in research; omit if absent>

  Rules: 2–3 OVER-PRICED lines max. Only flag claims that read as mainstream / repeated in research. Stop at 2 strong ones — do not pad.

UNDERPRICED: <narrative name>
  WHY MATTERS: <one sentence>
  SIGNAL: UNDERREPORTED
  MARKET: <Tier-6-style sharp-side hint from research if present; omit if absent>

  Rules: 1–2 UNDERPRICED lines. This section is REQUIRED if any UNDERREPORTED or contrarian-sourced facts appear in either research block. If none, write exactly:
    UNDERPRICED: none found in research pass.

No URLs, no hedging, no "on the other hand" balance — overpricing read only.
