...
async function generateResearchArtifacts(pick, { counter = false } = {}) {
  ...
  const rPrompt = counter ? counterResearchPrompt(grounding) : researchPrompt(grounding);
  ...
  return counter ? { counterNarrativeCombined: narrativeCombined } : { narrativeCombined };
}

export async function generateResearchNarrative(pick) {
  return generateResearchArtifacts(pick, { counter: false });
}

export async function generateCounterResearchNarrative(pick) {
  return generateResearchArtifacts(pick, { counter: true });
}
...
function userContextJudgeAddendum(userContext) {
  const block = sanitizeUserJudgeContext(userContext);
  if (!block) return "";
  return `

═══════════════════════════════════════════════════════
USER-SUPPLIED CONTEXT (not from Mahowny dossiers; not web-fetched here)
═══════════════════════════════════════════════════════
...
function judgePromptForBatch(picks, userContext) {
...
(No counter dossier was generated for this option — weigh only SUPPORTING prose + cites when comparing vs other picks that may have contra material.)`;
    ...
    body += `\n═══════════════════════════════════════════════════════
OPTION ${i + 1} · SLOT_INDEX: ${i} (RECOMMENDED_PICK_INDEX uses this integer — UI Pick #${i + 1})
═══════════════════════════════════════════════════════
${grounding}
...
RULES:
- Quote/paraphrase only from dossiers.
- Outside knowledge is not factual input for this verdict.
- Emit RECOMMENDED_PICK_INDEX as the best remaining ticket AFTER internal cross-examination — not merely “favorite team” intuition.

${body}
${userContextJudgeAddendum(userContext)}

REQUIRED FORMAT — verbatim first lines (machine-parsable):
RECOMMENDED_PICK_INDEX: <integer 0-${maxIx} — one SLOT_INDEX>
...
function judgePromptSingleTicketProCon(pick, userContext) {
  ...
  return `You balance ONE wager’s SUPPORTING dossier versus its CONTRARY dossier (same posted DraftKings line below). Texts come from earlier search-backed passes — you MUST NOT browse.
...
TASK (epistemics only — NOT a game-score prediction): After netting cited strength vs hype on BOTH wings, classify whether the dossiers collectively tilt toward taking this priced ticket versus passing/walking away. Penalize thin FACT rows and inflated prose.
${userContextJudgeAddendum(userContext)}

RESPONSE FORMAT — verbatim headers first:
RECOMMENDED_STANCE: <TAKE_TICKET | PASS_TICKET | TOO_CLOSE_TO_CALL>
CONFIDENCE: <low|medium|high>
...
function parseJudgeSingleTicketOutput(rawText) {
  ...
  const stanceMatch = text.match(/RECOMMENDED_STANCE:\s*(TAKE_TICKET|PASS_TICKET|TOO_CLOSE_TO_CALL)/i);
  const confMatch = text.match(/CONFIDENCE:\s*(low|medium|high)\b/i);
  const lineMatch = text.match(/ONE_LINE_SUMMARY:\s*([^\n]+)/i);

  const ticketStance = mapTicketStanceToken(stanceMatch ? stanceMatch[1] : null);
  ...
  for (let k = 0; k < 6; k++) {
    const next = reasoning
      .replace(/^\s*RECOMMENDED_STANCE:\s*[A-Z_a-z]+\s*\n?/im, "")
...
export async function judgeResearchedPicks(picks, options = {}) {
  ...
  if (picks.length === 1) {
    const prompt = judgePromptSingleTicketProCon(picks[0], options?.userContext);
    const res = await claudeWithRetry(claudeKey, () => ({
      model: judgeModel,
      max_tokens: userContextApplied ? 4200 : 3500,
      temperature: 0.14,
    ...
    return { ...parsed, model: judgeModel, userContextApplied };
  }
  ...
  const prompt = judgePromptForBatch(picks, options?.userContext);
  const res = await claudeWithRetry(claudeKey, () => ({
    model: judgeModel,
    max_tokens: userContextApplied ? 4500 : 4000,
    temperature: 0.15,
    messages: [{ role: "user", content: prompt }],
  }));
  const data = await res.json();
  ...
  return { ...parsed, model: judgeModel, pickCount: picks.length, userContextApplied };
}
