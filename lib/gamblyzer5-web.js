...
function filterEventsByEarliestEventUtcDay(events) {
  ...
  const m = earliest.getUTCMonth();
  const d = earliest.getUTCDate();
...
async function buildSortedCandidates({ league, leagues, min, max }) {
  ...
  for (let i = 0; i < leagueCodes.length; i++) {
    ...
    const dayEvs = filterEventsByEarliestEventUtcDay(data).map((ev) => ({ ev, leagueCode: code }));
    todays.push(...dayEvs);
  }
...
export async function judgeResearchedPicks(picks, options = {}) {
  ...
  if (picks.length === 1) {
    ...
    return { ...parsed, model: judgeModel, pickCount: 1, userContextApplied, judgeWebSearchEnabled: judgeAllowWeb };
  }
