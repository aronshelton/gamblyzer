...
function buildGroundingBlock(pick) {
  ...
  const lineMv = pick.oddsLineMovement ? `\n\n${pick.oddsLineMovement}` : "";
...
TIER 6 — Betting Market Structure (check on every bet)
  ...
  LINE MOVEMENT — use GROUNDING first (preferred)
  If GROUNDING includes "LINE MOVEMENT (The Odds API historical snapshots", that block is
  **authoritative feed data** for open-to-current price on this posted book/market/outcome.
  Emit one FACT with URL: https://the-odds-api.com/historical-odds-data/ | QUOTE: snapshot
  timestamp + earlier vs current American prices | SIGNAL: UNDERREPORTED.
  Do **not** web-search for line movement when this block is present.

  If GROUNDING has **no** historical line-movement block (optional fallback — best effort):
  Search in plain language for line movement / opening odds for this matchup (e.g. VegasInsider
  line movements, Odds Shark, or similar public line-history pages). Use whatever URL your
  search returns — do not construct or guess URL paths. GAP if nothing loads; never invent prices.

  PUBLIC BETTING SPLITS (optional — best effort)
  Try to find public % tickets vs % handle for this game (DraftKings Network betting splits or
  similar free split pages). If no reliable page loads for this matchup, GAP splits only —
  do not invent percentages. % Bets vs % Handle divergence is valuable when available.

  Do **not** use Action Network, Pregame, Bet Labs, Sports Insights, or
  sportsbook.draftkings.com for Tier 6 — they are inaccessible or subscription-gated.

  SIGNAL RULE: Tier 6 facts default to UNDERREPORTED unless pick aggregators already lead with
  the same split or line move today (then SATURATED).
`;
...
Tier 6 line movement from GROUNDING (Odds API historical): cite
https://the-odds-api.com/historical-odds-data/ — do not search the web for movement when that
block is already in GROUNDING.
...
- UNDERREPORTED = sourced from Tier 2–5 primary source; absent from aggregators
  (Tier 6: UNDERREPORTED when citing feed historical line movement or fresh split data unless
  aggregators already lead with the same move/split today — then SATURATED)
...
OVER-PRICED: <narrative name>
  ...
  SIGNAL: SATURATED
  MARKET: <line movement or betting-split note from research if any; omit if none>
...
UNDERPRICED: <narrative name>
  ...
  Line-movement or split findings from research should appear here when present.
...
function bucketToMarketKey(bucket) {
  if (bucket === "moneyline") return "h2h";
  if (bucket === "spread") return "spreads";
  if (bucket === "total") return "totals";
  return null;
}

function historicalLineMovementEnabled() {
  return String(process.env.GAMBLYZER_HISTORICAL_LINE_MOVEMENT || "1").trim() !== "0";
}

function historicalLookbackHours() {
  const raw = Number(process.env.GAMBLYZER_HISTORICAL_LOOKBACK_HOURS);
  if (!Number.isFinite(raw) || raw <= 0) return 24;
  return Math.min(72, Math.max(6, raw));
}

/** The Odds API historical `date` must be ISO8601 without fractional seconds (docs use 2021-10-18T12:00:00Z). */
function formatOddsApiHistoricalDate(d) {
  const t = d instanceof Date ? d : new Date(d);
  if (Number.isNaN(t.getTime())) return null;
  const fiveMin = 5 * 60 * 1000;
  const rounded = new Date(Math.floor(t.getTime() / fiveMin) * fiveMin);
  return rounded.toISOString().replace(/\.\d{3}Z$/, "Z");
}

function isInvalidHistoricalDateError(err) {
  const m = String(err?.message || err);
  return m.includes("422") && /invalid date/i.test(m);
}

/** Candidate snapshot times (newest-first), all strictly in the past and 5-min aligned. */
function historicalSnapshotCandidateDates(pick) {
  const now = Date.now();
  const minAgeMs = 10 * 60 * 1000;
  const hours = historicalLookbackHours();
  const raw = [
    now - hours * 60 * 60 * 1000,
    now - Math.max(6, Math.floor(hours / 2)) * 60 * 60 * 1000,
    now - 6 * 60 * 60 * 1000,
  ];
  const commence = pick?.fixture?.startTime ? new Date(pick.fixture.startTime).getTime() : NaN;
  if (Number.isFinite(commence)) {
    raw.push(commence - 2 * 60 * 60 * 1000, commence - 12 * 60 * 60 * 1000);
  }
  const seen = new Set();
  const out = [];
  for (const ms of raw) {
    const capped = Math.min(ms, now - minAgeMs);
    const iso = formatOddsApiHistoricalDate(new Date(capped));
    if (iso && !seen.has(iso)) {
      seen.add(iso);
      out.push(iso);
    }
...
function formatAmericanForDisplay(am) {
  const v = parseAmericanOdds(am);
  if (v == null) return String(am || "—");
  return v > 0 ? `+${v}` : `${v}`;
}

function describeAmericanOddsMove(priorAm, currentAm) {
  const prior = formatAmericanForDisplay(priorAm);
  const cur = formatAmericanForDisplay(currentAm);
  const a = parseAmericanOdds(priorAm);
  const b = parseAmericanOdds(currentAm);
  if (a == null || b == null) return `was ${prior}, now ${cur}`;
  if (a === b) return "unchanged";
  return `moved from ${prior} to ${cur}`;
}

function extractPostedBetLineFromHistoricalEvent(event, pick) {
  const rows = flattenCoreLinesFromTheOddsApi(event, { oddsFormat: "american" });
  const book = String(pick?.dkRow?.book || "").toLowerCase();
  const key = computeComparisonKey(pick.dkRow);
  const row = rows.find((r) => r.book === book && computeComparisonKey(r) === key);
  if (!row) return null;
  return { american: row.american || "", outcome: row.outcome };
}

async function fetchHistoricalEventOddsSnapshot(key, sportKey, eventId, dateIso, markets, regions) {
  const path =
    `/historical/sports/${encodeURIComponent(sportKey)}/events/${encodeURIComponent(eventId)}/odds` +
    `?regions=${encodeURIComponent(regions)}&markets=${encodeURIComponent(markets)}` +
    `&oddsFormat=american&date=${encodeURIComponent(dateIso)}`;
  return oddsApiGet(key, path);
}

async function fetchHistoricalSportOddsSnapshot(key, sportKey, dateIso, markets, regions) {
  const path =
    `/historical/sports/${encodeURIComponent(sportKey)}/odds` +
    `?regions=${encodeURIComponent(regions)}&markets=${encodeURIComponent(markets)}` +
    `&oddsFormat=american&date=${encodeURIComponent(dateIso)}`;
  return oddsApiGet(key, path);
}

function isHistoricalEventNotFoundError(err) {
  const m = String(err?.message || err);
  return m.includes("404") && /event not found|invalid event/i.test(m);
}

/**
 * Featured markets (h2h, spreads, totals) use sport-level historical odds; the per-event
 * endpoint is for additional markets (props, etc.) — see the-odds-api.com/historical-odds-data/
 */
async function loadHistoricalEventSnapshot(apiKey, sportKey, eventId, dateIso, marketKey, regions) {
  const fromSportFeed = async () => {
    const { data: bulk } = await fetchHistoricalSportOddsSnapshot(
      apiKey,
      sportKey,
      dateIso,
      marketKey,
    ...
    const events = Array.isArray(bulk?.data) ? bulk.data : [];
    const ev = events.find((row) => String(row?.id) === eventId);
    if (!bulk?.timestamp || !ev) return null;
    return { timestamp: bulk.timestamp, event: ev };
  };

  const sportSnap = await fromSportFeed();
  if (sportSnap) return sportSnap;

  try {
    const { data } = await fetchHistoricalEventOddsSnapshot(
      apiKey,
      sportKey,
      eventId,
      dateIso,
    ...
    if (data?.timestamp && data?.data) return { timestamp: data.timestamp, event: data.data };
  } catch (e) {
    if (!isInvalidHistoricalDateError(e) && !isHistoricalEventNotFoundError(e)) throw e;
  }

  return fromSportFeed();
}

function buildLineMovementGroundingBlock(pick, { requestedDate, snapTs, prior }) {
  const bookLabel = sportsbookDisplayName(pick.dkRow?.book || sportsbookForLeagueCode(pick.sportLabel));
  const postedLabel = outcomeLabelForRow(pick.dkRow);
  const currentAm = pick.dkRow?.american || "—";
  const move = describeAmericanOddsMove(prior.american, currentAm);

  return `LINE MOVEMENT (The Odds API historical snapshots — feed; cite https://the-odds-api.com/historical-odds-data/):
- Lookback query: ~${historicalLookbackHours()}h window (requested ${requestedDate}; API snapshot ${snapTs})
- Earlier: ${bookLabel} ${marketLabelForRow(pick.dkRow)} · "${postedLabel}": ${formatAmericanForDisplay(prior.american)}
- Current (this ticket): ${formatAmericanForDisplay(currentAm)} at ${bookLabel}
- Move on posted side: ${move}`;
}

/**
 * Pulls one earlier snapshot for this event/book/market via The Odds API historical endpoint
 * (paid plans). Returns a grounding block string or "" on skip/failure.
 * @see https://the-odds-api.com/historical-odds-data/
 */
async function fetchOddsApiLineMovementBlock(pick) {
  if (!historicalLineMovementEnabled()) return "";
  const apiKey = process.env.ODDS_API_KEY;
  if (!apiKey) return "";

  const eventId = String(pick?.fixture?.fixtureId || "").trim();
  const sportKey = sportKeyForLeagueCode(pick?.sportLabel);
  const marketKey = bucketToMarketKey(pick?.dkRow?.bucket);
  if (!eventId || !sportKey || !marketKey) return "";

  const regions = String(process.env.GAMBLYZER_HISTORICAL_REGIONS || "us").trim() || "us";
  const dateCandidates = historicalSnapshotCandidateDates(pick);
  if (!dateCandidates.length) return "";

  let lastErr = null;
  for (const dateIso of dateCandidates) {
    try {
      const snap = await loadHistoricalEventSnapshot(
        apiKey,
        sportKey,
        eventId,
        dateIso,
      ...
      const prior = extractPostedBetLineFromHistoricalEvent(snap.event, pick);
      if (!prior?.american) continue;

      return buildLineMovementGroundingBlock(pick, {
      ...
      lastErr = e;
      if (!isInvalidHistoricalDateError(e) && !isHistoricalEventNotFoundError(e)) break;
    }
  }

  console.warn(
    "[gamblyzer] historical line movement skipped:",
    lastErr?.message || "no snapshot with matching line for this ticket"
  );
  return "";
}
...
function americanFromOutcomePrice(price) {
  const am = parseAmericanOdds(price);
  if (am != null) return am > 0 ? `+${am}` : `${am}`;
  return "";
}

function flattenCoreLinesFromTheOddsApi(eventOdds, { oddsFormat = "decimal" } = {}) {
  const rows = [];
  const fixture = fixtureFromOddsEvent(eventOdds);
  const home = fixture.participant1Name;
  const away = fixture.participant2Name;
  const pricesAreAmerican = String(oddsFormat).toLowerCase() === "american";
  ...
  for (const bm of (eventOdds?.bookmakers || [])) {
    ...
    for (const m of (bm?.markets || [])) {
      ...
      for (const o of (m?.outcomes || [])) {
        ...
        let dec = Number(rawPrice);
        let american = "";
        if (pricesAreAmerican) {
          american = americanFromOutcomePrice(rawPrice);
          if (!name || !american) continue;
          dec = Number.NaN;
        } else {
        ...
        const am = pricesAreAmerican ? parseAmericanOdds(rawPrice) : decimalToAmerican(dec);
        rows.push({
          book,
          bucket,
          marketName,
...
async function pickWithLineMovement(pick) {
  if (pick?.__lineMovementFetchDone) return pick;
  const oddsLineMovement = await fetchOddsApiLineMovementBlock(pick);
  return {
    ...pick,
    __lineMovementFetchDone: true,
    ...(oddsLineMovement ? { oddsLineMovement } : {}),
  };
}

async function runResearchLeg(pick, { counter, userContext, gapClosure, claudeKey, geminiKey, model }) {
  const pickForGrounding = await pickWithLineMovement(pick);
  const grounding = buildGroundingBlock(pickForGrounding);
...
function researchLineMovementExtra(pick) {
  return pick?.oddsLineMovement ? { oddsLineMovement: pick.oddsLineMovement } : {};
}

async function generateResearchArtifacts(pick, { counter = false, userContext = "", gapClosure = "" } = {}) {
  assertPickShape(pick);
  pick = await pickWithLineMovement(pick);
    ...
    if (nc.includes("<<<GAMBLYZER_SOURCES>>>")) {
      ...
      try {
        ...
        return {
          ...
          llmUsage,
          ...researchLineMovementExtra(pick),
        };
      } catch {
        /* bias is best-effort when counter is added after primary */
      }
    }
    return { counterNarrativeCombined: narrativeCombined, llmUsage, ...researchLineMovementExtra(pick) };
  }

  return { narrativeCombined, llmUsage, ...researchLineMovementExtra(pick) };
}
...
async function generateResearchArtifactsPrimaryWithBias(pick, { userContext = "", gapClosure = "" } = {}) {
  ...
  return { narrativeCombined, biasNarrative, llmUsage, ...researchLineMovementExtra(pick) };
}
...
/** Integration probe for historical line movement (scripts/test-historical-line-movement.mjs). */
export async function probeHistoricalLineMovement(pick) {
  assertPickShape(pick);
  return fetchOddsApiLineMovementBlock(pick);
}
