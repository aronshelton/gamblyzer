...
function buildGroundingBlockNarrative(pick) {
  ...
  const spreadHint =
    dkRow?.bucket === "spread"
      ...
      : ` | home=${h} away=${a}`;
  return `Feed facts (unchangeable): ${sportLabel} | ${fixture.participant2Name} @ ${fixture.participant1Name} | bet ${outcomeLabelForRow(dkRow)} / ${marketLabelForRow(dkRow)} | ${dkRow.american || "—"} (${Number.isFinite(dkRow.decimalOdds) ? dkRow.decimalOdds.toFixed(3) : "—"}) DraftKings${spreadHint}`;
}
...
TIER 2 — Process Metrics vs. Surface Stats
  MLB:  Baseball Savant — pitch mix shifts, xERA vs ERA gap, barrel%, exit velo trend
  NBA:  Cleaning the Glass or NBA.com/stats — shot quality, on/off splits (not box score)
  NHL:  Natural Stat Trick or MoneyPuck — GSAx, HDCA%, zone entry/exit rates
  ANY:  Find a gap between what the surface stat shows and what the process metric shows. This gap is almost never in aggregator pick content.

TIER 3 — Matchup-Specific Splits
  MLB:  Batter vs. pitcher handedness split; road/home ERA split (Baseball Savant)
  NBA:  Defender matchup data, pace differential, bench depth vs. this opponent (NBA.com)
  NHL:  Line matchup deployment, PP vs. PK efficiency head-to-head (Natural Stat Trick)
  ANY:  Find a split specific to THIS matchup — not just season-wide numbers.

TIER 4 — Environmental / Schedule / Contextual Factors
  MLB:  UmpScorecards (umpire zone tendency), wind/temp at ballpark (weather.com)
  NBA:  Back-to-back or 3-in-4 flag, travel direction (west-to-east fatigue)
  NHL:  Back-to-back flag, goalie starts in last 5 days, home/away save% split
  ANY:  Rest differential, travel burden, schedule spot — things preview sites routinely skip.

TIER 5 — Recent Game Log Trend
  ANY:  Last 2–3 game logs for the key player/pitcher/goalie from official box score source.
        Look for a short-term trend NOT captured in season averages.
...
TIER 6 — Betting Market Structure (check on every bet)
  SOURCES (priority order):
    ...
    DraftKings Sportsbook               — live consensus bet % for this matchup
  ...
  WHAT TO LOOK FOR:
    - Bet % vs. Money % divergence: if 70% of bets are on one side but only 45% of
      money is — large/sharp bettors are on the other side.
    - Reverse line movement (RLM): line moves AGAINST the public side despite lopsided
      bet % (e.g. 75% public on a team but line moves in their favor to shorter odds).
      Strong sharp indicator.
    - Line open vs. current: movement of 10+ cents since open = meaningful one-sided
      action; note direction.
    - Consensus threshold: >65% public on one side = meaningful lean;
      >80% = potential fade candidate.
`;
...
function judgePromptSingleTicketProCon(pick, userContext, allowWeb) {
  ...
  return `You balance ONE wager’s **full dossier**: SUPPORTING case, CONTRARY case, and BIAS read (same posted DraftKings line below). ${webRule}
